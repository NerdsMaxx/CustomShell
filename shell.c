...
void command_interpreter(char *line, int length_line){
    ...
    //Se is_background for verdadeiro, chama signal
    //e executar a função do waitpid para evitar
    //processo zumbi por parte do processo filho.
    if (is_background){
        change_option_for_wait(1);
        signal(SIGCHLD, wait_child_pid);
    }
    else {
        //Senão espere normalmente por processo filho terminar.
